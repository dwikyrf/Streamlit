...
def create_byreview_df(df):
    byreview_df = df.groupby(by="review_score").review_id.nunique().reset_index()
...
st.subheader("Total Review By Customer")




sns.set_style("whitegrid")
st.title("Total Review By Customer")
fig, ax = plt.subplots(figsize=(6, 6))
ax.pie(byreview_df['customer_count'], labels=byreview_df['review_score'], autopct='%1.1f%%')
ax.set_title('Total Review by Customer')
st.pyplot(fig)

sns.set_style("whitegrid")

# Streamlit code
st.title("Number of Status by Customer")
...
ax.set_title("Number of Status by Customer", loc="center", fontsize=15)
ax.set_ylabel(None)
ax.set_xlabel(None)
...
st.caption('Copyright (c) E-Commerce 2023')